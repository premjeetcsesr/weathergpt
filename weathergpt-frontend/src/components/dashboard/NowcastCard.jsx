      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
        {nowcastData.points.slice(0, 4).map((pt, idx) => {
          ...
          return (
            <div
              key={idx}
              className="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60 text-center space-y-1 text-xs"
            >
              <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 block">
                {timeLabel}
              </span>
              <div className="flex items-center justify-center gap-1 font-bold text-slate-800 dark:text-slate-200">
                <CloudRain className={`w-3.5 h-3.5 ${isHighPop ? 'text-blue-500' : 'text-slate-400'}`} />
