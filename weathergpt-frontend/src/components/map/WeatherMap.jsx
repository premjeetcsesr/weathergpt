...
export function WeatherMap({ height = "550px", focusCoords = null, focusReportId = null }) {
  ...
  const userLat = weatherData?.location?.lat;
  const userLon = weatherData?.location?.lon;
    ...
    <div className="relative w-full rounded-3xl overflow-hidden shadow-card border border-slate-200/80 dark:border-slate-800 bg-slate-900" style={{ height }}>
      ...
      <div className="absolute top-2 sm:top-4 left-2 sm:left-4 right-2 sm:right-4 z-[400] flex flex-col gap-2 pointer-events-none">
        <div className="pointer-events-auto flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5 max-w-full">
          ...
          />
          {['hospital', 'pharmacy', 'emergency_room', 'shop'].map((category) => (
            <button
              key={category}
              type="button"
              onClick={() => setNearbyCategory((current) => current === category ? null : category)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-sm font-bold shadow-elevated shrink-0 ${
                nearbyCategory === category ? 'bg-rose-500 text-white' : 'bg-white/95 dark:bg-slate-900/95 text-slate-700 dark:text-slate-200'
              }`}
            >
              {category === 'hospital' && <Hospital className="w-4 h-4" />}
              {category === 'pharmacy' && <Pill className="w-4 h-4" />}
              {category === 'emergency_room' && <ShieldAlert className="w-4 h-4" />}
              {category === 'shop' && <Store className="w-4 h-4" />}
              {category === 'hospital' ? 'Hospitals' : category === 'pharmacy' ? 'Pharmacies' : category === 'emergency_room' ? 'Emergency' : 'Shops'}
            </button>
          ))}
          <button
            type="button"
            onClick={() => setShowCommunityReports(!showCommunityReports)}
            className={`px-4 py-2.5 rounded-2xl text-sm font-bold transition-all shadow-elevated flex items-center gap-2 shrink-0 ${
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-2xl text-sm font-bold transition-all shadow-md flex items-center gap-2 bg-gradient-to-r from-red-500 via-amber-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white shrink-0"
          >
            <Camera className="w-3.5 h-3.5" />
            <span>{t('reportIncidentBtn') || 'Report Hazard'}</span>
          </button>
        </div>
      ...
      {nearbyCategory && (
        <aside className="absolute right-3 top-32 z-[400] w-[min(22rem,calc(100%-1.5rem))] max-h-[min(28rem,calc(100%-9rem))] overflow-y-auto rounded-3xl border border-slate-200 bg-white/95 p-4 shadow-2xl backdrop-blur-md dark:border-slate-700 dark:bg-slate-900/95">
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Within 5 km of your location</p>
            </div>
            <LocateFixed className="h-5 w-5 text-brand-500" />
          </div>
          {isLoadingNearby && <p className="py-4 text-center text-sm text-slate-500">Finding nearby places...</p>}
          ...
          <div className="space-y-2">
            {nearbyPlaces.map((place) => {
              const distance = userLat && userLon && place.latitude && place.longitude
                ? getDistanceKm(userLat, userLon, place.latitude, place.longitude)
                : null;
              return (
                <div key={place.id} className="rounded-2xl border border-slate-200 p-3 dark:border-slate-700">
                  <div className="flex items-start gap-2">
                    <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-rose-500" />
                    <div className="min-w-0 flex-1">
                      <h4 className="truncate text-sm font-bold text-slate-900 dark:text-white">{place.name}</h4>
                      <p className="mt-1 line-clamp-2 text-xs text-slate-500 dark:text-slate-400">{place.address}</p>
                      <div className="mt-2 flex items-center justify-between gap-2">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 rounded-xl bg-brand-600 px-2.5 py-1.5 text-xs font-bold text-white hover:bg-brand-700"
                        >
