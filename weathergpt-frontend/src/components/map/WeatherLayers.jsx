...
export function WeatherLayers({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 p-2 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-elevated max-w-full">
      <div className="flex items-center gap-1.5 px-2 text-sm font-semibold text-slate-500 dark:text-slate-400 shrink-0">
      ...
      {layers.map((layer) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-bold transition-all shrink-0 ${
              isActive
                ? 'bg-brand-500 text-white shadow-sm'
                : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
            }`}
          >
            <Icon className={`w-4 h-4 ${isActive ? 'text-white' : layer.color}`} />
