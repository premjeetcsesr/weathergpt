...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 transition-colors pt-[max(env(safe-area-inset-top,0px),0.25rem)]">
      <div className="w-full px-3 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5">
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="hidden md:block p-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-300 transition-colors"
            title="Toggle theme"
          >
            {theme === 'dark' ? (
      ...
      {showMobileNavMenu && (
        <>
          ...
          <div className="md:hidden fixed top-14 left-0 right-0 z-50 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-2xl p-4 max-h-[85vh] overflow-y-auto animate-in slide-in-from-top duration-200">
            ...
            {/* Quick Preferences Toggles Row */}
            <div className="grid grid-cols-2 gap-2 mb-3">
              {/* Temp Unit Toggle */}
              <button
                type="button"
                onClick={toggleTempUnit}
                className="flex items-center justify-between p-2.5 rounded-2xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300"
              >
                <span>Temperature</span>
                <span className="px-2 py-0.5 rounded-lg bg-brand-500 text-white font-bold text-[11px]">
                  °{tempUnit}
                </span>
              </button>
