...
export function MobileNav() {
  ...
  return (
    <>
      ...
      <div
        className={`md:hidden fixed bottom-[calc(4.25rem+env(safe-area-inset-bottom))] left-3 right-3 z-50 rounded-3xl border border-[#1a2c4e] bg-[#0c1527] p-3.5 shadow-2xl transition-all duration-200 ${
          isMenuOpen ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-4 opacity-0'
        }`}
        aria-hidden={!isMenuOpen}
      >
        <div className="flex items-center gap-2 px-2 pb-2.5 mb-2 border-b border-[#1a2c4e]">
          <Sparkles className="w-4 h-4 text-brand-500" />
          <span className="text-xs font-bold text-white">WeatherGPT Quick Menu</span>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0c1527]/95 backdrop-blur-lg border-t border-[#1a2c4e] px-2 pt-1 pb-[calc(0.375rem+env(safe-area-inset-bottom))] shadow-elevated">
